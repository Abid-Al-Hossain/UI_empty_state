# UI Foundry Empty State Studio

Standalone visual Empty State generator built with Next.js, React, and TypeScript.

Standalone Empty State component studio for building native, preview-honest, React-exportable UI patterns that match the UI Foundry product line.
...
- designers building Empty State UI quickly
- developers who need editable React starting points
- marketplace buyers who want a self-contained Empty State generator
...
The editor is focused on controls that are native to Empty State and useful in real product work:
- title, description, icon/illustration, primary/secondary action, helper, and layout
- surface, spacing, alignment, typography, color, radius, border, and shadow controls
- search, error, onboarding, empty list, and permissions archetypes
- semantic messaging and accessible action labels
- React export for empty state composition

The goal is maximum useful depth without adding unrelated controls that would make the component stop reading as Empty State.
...
This repo is intended to publish as Abid-Al-Hossain/UI_empty_state.
